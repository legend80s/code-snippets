...
export function convertProLayoutRoutesToReactRoutes(proLayoutRoute) {
  ...
  function processRoute(route) {
    ...
    if (route.component) {
       ...
       * @param {string} name
       */
      const replaceInvalidName = name => name.replace('404', 'NotFound').replace('.tsx', '');

      // ../layouts/ and ../components/
      if (route.component.startsWith('../layouts/')) {
        componentName = route.component.replace('../layouts/', '').split('/').pop();
        importPath = '@/layouts/' + componentName;
      } else if (route.component.startsWith('../components/')) {
        componentName = route.component.replace('../components/', '').split('/').pop();
        importPath = '@/components/' + route.component.replace('../components/', '');
...
const isMainModule = import.meta.url.startsWith(pathToFileURL(process.argv[1]).href);
...
async function main() {
  ...
  const debouncedRegenerate = debounce(generate);
