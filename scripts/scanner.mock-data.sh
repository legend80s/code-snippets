...
if [ "$SRC_FILE_COUNT" -eq 0 ]; then
    ...
    success "✅ 无 src/ 目录文件变更，跳过。耗时: ${duration} 毫秒。"
    exit 0
fi

info "🔍 扫描 ${SRC_FILE_COUNT} 个 src/ 目录文件..."
        ...
        if [[ -n "$DIFF_CONTENT" ]]; then
            ...
            if [ "$HAS_SUCCESS" = true ] && [ "$HAS_CODE" = true ] &&
               [ "$HAS_ERRORMSG" = true ] && [ "$HAS_DATA" = true ]; then
                error "🚨 [MOCK_DATA_SCANNER] 发现危险的 Mock 数据: $FILE"
...
success "✅ Mock 数据检查通过 (共扫描 ${SRC_FILE_COUNT} 个文件)，耗时: ${duration} 毫秒。"
